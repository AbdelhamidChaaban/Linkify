  ...
  try {
    ...
    if (adminDbInstance) {
      try {
        ...
        // Note: This works with orderBy on the same field without needing a composite index
        if (minTimestamp) {
          query = query.where('timestamp', '>=', admin.firestore.Timestamp.fromDate(minTimestamp));
        }
        
        // Add action filter BEFORE orderBy to avoid index issues when possible
        // If it causes an index error, we'll catch it and use fallback
        if (actionFilter !== 'all') {
        ...
        console.log(`✅ Retrieved ${logs.length} action log(s) for user ${userId} (actionFilter: ${actionFilter}, dateFilter: ${dateFilter}) using Admin SDK`);
        return logs;
      } catch (indexError) {
